...
Splicesite:
        ...
        pyhton3 $(hisat2_extract_splice_sites) $(gtf) >$(outdir)/splicesites.txt
...
Alignment:
        ...
        then \
                ...
                hisat2 -x $(genome_index) -p $(cpu) --dta $(strandness) -U $(fq1) --known-splicesite-infile $(known_splicesites) -temp-directory $(outdir)/tmp 2> $(outdir)/$(sample).hisat2.stderr| samtools sort -@ $(cpu) -m 768M > $(outdir)/$(sample).bam ;fi 
